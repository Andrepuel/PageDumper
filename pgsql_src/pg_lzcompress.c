...
#define giveup_assertion(cond,msg) if( !(cond) ) { /*printf(msg);*/ dest[0] = 'c'; dest[1] = 'c'; return; };
	...
	while (sp < srcend && dp < destend)
	{
		...
		for (ctrlc = 0; ctrlc < 8 && sp < srcend; ctrlc++)
		{
			if (ctrl & 1)
			{
				...
				while (len--)
				{
					//WORKAROUND to keep the code from writing invalid address
					giveup_assertion( (dp-off) >= dest, "corrupted data, compression tried looking backward" );
					*dp = dp[-off];
					dp++;
				}
			}
...
//		WORKAROUND: Ignore this situation because it appers that the decompressed data is right
//		printf("warning, remaining data on input\n");
	}
	if (dp < destend && sp != srcend) {
		printf("compressed data is corrupt, %d %d\n", (destend-dp), (srcend-sp));
		abort();
